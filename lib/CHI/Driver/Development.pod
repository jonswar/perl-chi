...
CHI driver classes are Moose based, so you should read (or at least have available) the
documentation for Moose before proceeding.
...
=item _remove ( $self, $key )

Remove the data associated with the I<$key> in the namespace. This is called by remove().
...
Override this if you want to provide an efficient method of clearing a namespace.  This is
called by clear(). The default implementation will iterate over all keys and call
remove() for each.
...
keys. This is called by get_keys().

=item _get_namespaces ( $self )

Return namespaces associated with the cache. It is acceptable to either include or omit
namespaces with no valid keys. This is called by get_namespaces().
