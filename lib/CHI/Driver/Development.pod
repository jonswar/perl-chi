...
CHI driver classes are L<Moose|Moose> based. We recommend that you also use
L<Moose> for consistency and for its many features, but since Moose plays
well with standard OO classes, you are not required to do so.
...
Define the Moose BUILD method if you want to process any options specific
to your driver.
