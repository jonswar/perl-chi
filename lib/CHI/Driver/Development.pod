...
CHI driver classes are L<Any::Moose|Any::Moose> based, meaning that they will
use L<Moose|Moose> if it is loaded, otherwise L<Mouse|Mouse>. We recommend that
you also use L<Any::Moose> for consistency and clarity, but since Moose/Mouse
play well with standard OO classes, you are not required to do so.
...
Define the Moose/Mouse BUILD method if you want to process any options specific
to your driver.
