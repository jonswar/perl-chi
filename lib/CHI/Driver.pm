...
#
my @common_params = map { $_->meta->get_attribute_list() } ( 'CHI::Driver',
...
sub unpack_from_data {
      ...
      return CHI::CacheObject->unpack_from_data( $key, $data,
          $self->serializer );
}
...
sub set_with_options {
      ...
      #
      my $obj = CHI::CacheObject->new( $key, $value, $created_at,
          $early_expires_at,
...
my $_fail_hi = sub {
      croak( sprintf "Can't escape multibyte character \\x{%04X}",
          ord( $_[0] ) );
  };
...
sub is_escaped_for_filename {
      ...
      return $self->escape_for_filename( $self->unescape_for_filename(
              $key) ) eq $key;
}
...
sub _handle_get_error {
      ...
      my $msg = sprintf( "error during %s: %s", $self->_describe_cache_get(@_),
          $error );
...
sub _handle_set_error {
      ...
      my $msg = sprintf( "error during %s: %s",
          $self->_describe_cache_set($obj),
          $error );
...
sub _describe_cache_set {
      ...
      my $expires_str =
        ( ( $obj->expires_at == CHI_Max_Time )
          ? 'never'
          : Time::Duration::concise( Time::Duration::duration_exact(
